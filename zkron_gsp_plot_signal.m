function zkron_gsp_plot_signal(G,signal,param,axs)
...
if size(G.coords,2) == 2
    if param.bar
        ...
        %a = [1:1:length(signal)]';
        % If the graphs have the same size!
        ...
        c = cellstr(b);
        dx = 0.01; dy = 0.01;
        text(G.coords(:,1)+dx, G.coords(:,2)+dy, c);
